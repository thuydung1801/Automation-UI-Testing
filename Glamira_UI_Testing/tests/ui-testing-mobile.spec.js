...
urls.forEach(url => {
  test("listing select styles: " + url, async ({ playwright }) => {
    const browser = await playwright.chromium.launch({
      args: ['--remote-debugging-port=9222'],
    })
    const context = await browser.newContext();
    const page = await context.newPage()
    await page.goto(url + 'diamond-rings/flower/?imgps=2');
    await page.waitForTimeout(5000)
...
urls.forEach(url => {
  test("listing more categories: " + url, async ({ playwright }) => {
    const browser = await playwright.chromium.launch({
      args: ['--remote-debugging-port=9222'],
    })
    const context = await browser.newContext();
    ...
    await page.evaluate(() => window.scrollTo(900, 0))
    const more_categories = "#top_category > div.tab-title.more-category-title > a";
    await page.waitForSelector(more_categories)
    await page.click(more_categories)
...
urls.forEach(url => {
  test("product initals ring page: " + url, async ({ playwright }) => {
    ...
    await page.click("#associate_option_initial > div.option_label.inactive > span.option_label_title")
    await page.waitForTimeout(5000)
    const language = page.locator(
      "#geoip-detect > div.geoip-detect-right > div > div.geoip-wrapper-content > a.btn-stay-here.geoip-close"
    )
...
urls.forEach(url => {
  test("landing rings home: " + url, async ({ playwright }) => {
    ...
    const context = await browser.newContext();
    const page = await context.newPage()
    await page.goto(url+"rings-home/");
    await page.waitForTimeout(5000)
    // visually comparing two screenshots
    await expect(page).toHaveScreenshot(
      {
...
urls.forEach(url => {
  test("checkout " + url, async ({ playwright }) => {
    ...
    await page
      .locator(
        'button.action.primary.delete-cart'
      )
      .click()
  });
});
...
urls.forEach(url => {
  test("login forgot and create account: " + url, async ({ playwright }) => {
    ...
    //click back
    await page.locator(
      "#html-body > div.modals-wrapper > aside.modal-slide.modal-sm.modal-sm-fullscreen.navigation-drawer.offcanvas.forgot-password-modal._inner-scroll.show-back._show > div.modal-inner-wrap > header > button.action-back"
      ).click();
    await page.waitForTimeout(4000);
    //await page.locator('#download-app-popup > div > div.arown-down').click();
    //click create btn
    await page.locator("#customer_form_login_form_ajax > div.footer-additional-information > div > a > span").click();
    await page.waitForTimeout(4000);
    await expect(page).toHaveScreenshot(
      {
        timeout: 50000, maxDiffPixelRatio: 0.2
